...
Generated by `python scripts/build_final_scores.py` from `experiments/final/manual_review_scores.json` plus the tracked raw April 8 run JSONs.
...
The prompt-level `Published Phase 1`, `Challenge Review`, and `Published` values below are complete for all 30 prompts and are generated from a machine-readable manual-review manifest plus the tracked raw April 8 run JSONs.
